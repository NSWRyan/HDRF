...
public class Block implements Writable, Comparable<Block> {
  ...
  public void writeLog(String text){//Plastamod
    File fout = new File("/home/hadoop/out.txt");
    try {
      fout.createNewFile();
    ...
    writeLog("before numBytes = "+numBytes);
    if(numBytes==0){
      utilities ut1 = new utilities();
      Jedis jedis = new Jedis("localhost");
      byte[] t1data = jedis.get(ut1.longToBytes(blockId, 4));
      if (t1data != null) {
        byte[] fsize = new byte[4];
        System.arraycopy(t1data, 0, fsize, 0, 4);
        this.numBytes = ut1.bytesToLong(fsize, 4);
        fsize=null;
      }
      ut1=null;
      t1data=null;
      jedis.close();
      jedis = null;
      writeLog("numBytes = "+numBytes);
    }
    writeLog("after numBytes = "+numBytes);
  ...
  public void setNumBytes2(long len) {
    this.numBytes = len;
  }
